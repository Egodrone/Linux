...
#ANSWER = $(wc -l $FILE)
ANSWER=$(bro="wc -l ircLog.txt" && eval "$bro")
...
ANSWER=$(yoyo="wc -w ircLog.txt | cut -b 1-5" && eval "$yoyo")
...
ANSWER=$(zebra="cat ircLog.txt | grep notepad" && eval "$zebra")
...
ANSWER=$(ufc="tail -4 ircLog.txt" && eval "$ufc")
...
ANSWER=$(therp="head -1 ircLog.txt" && eval "$therp")
...
ANSWER=$(okok="cat ircLog.txt | grep -m3 'wasa' | tail -n1" && eval "$okok")
...
#ANSWER=$(truii="cat ircLog.txt | grep '11:15'" && eval "$truii")
ANSWER=$(truii="cat ircLog.txt | grep -o '11:15' | wc -l" && eval "$truii")
...
ANSWER=$(truii2="cat ircLog.txt | grep '07:48' | grep -m1 'pansar' | tail -n1" && eval "$truii2")
...
ANSWER=$(huhi="cat ircLog.txt | grep -A 10 'Wed Jun 17 2015' | tail -n10" && eval "$huhi")

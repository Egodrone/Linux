...
var server = "http://" + LINUX_SERVER + ":"+ LINUX_PORT;
...
function menu() {
 ...
 place random    Place random market on the board..
...
rl.on("line", function(line) {
    ...
    switch (args[0]) {
        ...
        case "place":
            ...
            if (args[1] == "random") {
                var x = Math.floor(Math.random() * (msize - 0 + 1)) + 0;
                var y = Math.floor(Math.random() * (msize - 0 + 1)) + 0;
                console.log(x);
                gomoku.place(x, y)
                    .then(value => {
                        console.log(value);
